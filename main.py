...
from src.encrypted.encrypt import *
import threading
import time
import socket
import json
...
shared_0500_info = {"got": False, "idT": None, "squad": None, "AutH": None}

MASTER_ACCOUNT_ID = "4606639184"
...
def parse_ghost_names(ghost_names_param):
    """Parse ghost names from {ghost1}{ghost2}{ghost3} format"""
    if not ghost_names_param:
        return []
    
    # Extract names between curly braces
...
class TcpBotConnectMain:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    if self.account_id == MASTER_ACCOUNT_ID:
                                        ...
                                        shared_0500_info["idT"] = self.packet["5"][
                                            "data"
                                        ]["1"]["data"]
                                        shared_0500_info["squad"] = self.packet["5"][
                                            "data"
                                        ]["31"]["data"]
                                        shared_0500_info["AutH"] = self.AutH
                                        print(
                                            f"[{self.account_id}] Master saved 0500 info"
                                        )
                                        ...
                                        for _ in range(3):
                                            self.socket_client.send(
                                                GenJoinSquadsPacket(idT, key, iv)
                                            )
                                            time.sleep(0.5)
                                            self.socket_client.send(
                                                ExiT("000000", key, iv)
    ...
    def guest_token(self, uid, password):
        ...
        headers = {
            "Host": "ffmconnect.ggwhitehawk.com",
    ...
    def TOKEN_MAKER(
        ...
        data = data.replace(b"1.114.6", b"1.118.1")
    ...
    def execute_command(self, command, *args):
        ...
        if "/XRRR" in command[:5]:
            try:
                ...
                print(
                    f"[{self.account_id}] Executing /XRRR for team code {self.id} with name {self.nm}"
                )
                ...
                if self.account_id == MASTER_ACCOUNT_ID:
                    ...
                    while not got_0500 and attempt_counter < 200:
                        attempt_counter += 1
                        print(
                            f"[{self.account_id}] Attempt {attempt_counter} joining/exiting squad {self.id}..."
                        )

                        self.socket_client.send(
                            GenJoinSquadsPacket(self.id, self.key, self.iv)
                        )
                        time.sleep(0.1)
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    for _ in range(1):
                                        self.socket_client.send(
                                            ghost_pakcet(
                                                idT, self.nm, sq, self.key, self.iv
                                            )
                                        )
                                        time.sleep(0.1)
                                    self.socket_client.send(
                                    ...
                                    print(
                                        f"[{self.account_id}] 0500 packet received but keys missing, skipping parse."
                                    )
                            except Exception as parse_err:
                                print(
                                    f"[{self.account_id}] Error parsing 0500: {parse_err}"
                    ...
                    return f"/XRRR master command executed successfully"
                    ...
                    return f"/XRRR ghost command executed using master data"
...
def home():
    return jsonify({
        "status": "online",
        "service": "FreeFire Bot API",
        "endpoints": {
            "/start_client": "GET ?account_id=ID&password=PASS",
            "/stop_client": "GET ?account_id=ID",
            "/list_clients": "GET",
            "/execute_command": "GET ?command=/XRRR=CODE&name=NAME",
            "/execute_command_all": "GET ?command=/XRRR=CODE&ghost_names={name1}{name2}",
            "/shutdown": "GET"
        }
    })
...
def execute_command_all():
    ...
    results = {}
    for idx, (account_id, client) in enumerate(clients.items()):
        if idx < len(ghost_names_list):
            account_name = ghost_names_list[idx]
        else:
            # Fallback to stored ghost names or account ID
            account_name = ghost_names.get(str(account_id), str(account_id))
            
        if cmd == "/XRRR" and arg:
            result = client.execute_command(cmd, arg, account_name)
            results[account_id] = f"Dev: Wotax | {result} | Name: {account_name}"
        else:
            results[account_id] = (
                f"Unknown or invalid command: {command} | Name: {account_name}"
            )
